...
/**
 * @author Tobse Overlay to easily acces a {@link SavedState} without catching exeptions and
 *         with <code>boolean</code> support. Local data is loaded when
 *         {@link #setFile(String)} is called and will be saved with ervery set function.
 *         Loading or saving exeptions will only trigger {@link Log} warnings.
 */
public class MyPropertys {

	/**
	 * Iis used to store the options local.
	 */
	...
	/**
	 * Stores string values which are setted before calling {@link #setFile(Class)} or
	 * {@link #setFile(String)}
	 */
	private static Properties entriesBeforeSettingFile = new Properties();

	/** Indicates if field names schould be checked for default with {@link #checkForDefaults} */
	 ...
	 * {@link #checkArg(String)} gets a not registed arg, if then also an exeption should be
	 * thrown.
	 */
	 ...
	 * 
	 * @param properties which schould be stored
	 */
	...
	/**
	 * Saves the options local, without throwing an exeption
	 */
	...
	/**
	 * Loads the options local, without throwing an exeption
	 */
	...
	/**
	 * Checks if the arg is a registered option of {@link DefaultArgs} or if setted of
	 * {@link #defaultArgs}. So only intended values can be setted and returend.
	 * 
	 ...
	 * registred and if {@link #throwExeptonOnArgsCheking} is <code>true</code>, also an
	 ...
	 * 
	 * @param throwExeptonOnArgsCheking <code>true</code> if not registed args schlould be also
	 *            throw an exeption in {@link #checkForDefaults} -not only a Log waring
	 * @see #checkArg(String)
	 */
