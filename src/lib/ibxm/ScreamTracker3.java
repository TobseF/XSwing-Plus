...
public class ScreamTracker3 {
	private static final int[] effect_map = new int[] { 0xFF, 0x25, /*
																	 * A: Set Speed.
																	 */
	0x0B, /* B: Pattern Jump. */
	...
	0x07, /* R: Tremolo. */
	0x0E, /* S: Extended EffectCatalog. */
	0x0F, /* T: Set Tempo. */
	0x24, /* U: Fine Vibrato. */
	0x10, /* V: Set Global Volume. */
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };

	private static final int[] effect_s_map = new int[] { 0x00, /*
																 * 0: Set Filter.
																 */
	0x03, /* 1: Glissando. */
	...
	public static Module load_s3m(byte[] header_96_bytes, DataInput data_input)
			throws IOException {
		...
		for (pattern_idx = 0; pattern_idx < num_patterns; pattern_idx++) {
			module.set_pattern(pattern_idx, read_s3m_pattern(s3m_file, pattern_idx,
					channel_map));
		}
		return module;
	}
	...
	private static Instrument read_s3m_instrument(byte[] s3m_file, int instrument_idx,
		...
		if (s3m_file[instrument_offset] == 1) {
			...
			if (s3m_file[instrument_offset + 30] != 0) {
				throw new IllegalArgumentException(
						"ScreamTracker3: Packed samples not supported!");
			}
	...
	private static byte[] read_s3m_file(byte[] header_96_bytes, DataInput data_input)
			throws IOException {
		int s3m_file_length;
	...
	private static byte[] read_more(byte[] old_data, int new_length, DataInput data_input)
			throws IOException {
		byte[] new_data;
		new_data = old_data;
		if (new_length > old_data.length) {
			...
			try {
				data_input.readFully(new_data, old_data.length, new_data.length
						- old_data.length);
