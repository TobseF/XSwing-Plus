...
public class FastTracker2 {
	...
	public static Module load_xm(byte[] header_60_bytes, DataInput data_input)
			throws IOException {
		...
		if (xm_version != 0x0104) {
			throw new IllegalArgumentException("Sorry, XM version " + xm_version
					+ " is not supported!");
		}
		module = new Module();
	...
	private static Pattern read_xm_pattern(DataInput data_input, int num_channels)
			throws IOException {
		...
		if (packing_type != 0) {
			throw new IllegalArgumentException("Pattern packing type " + packing_type
					+ " is not supported!");
		}
		pattern = new Pattern();
	...
	private static Instrument read_xm_instrument(DataInput data_input) throws IOException {
		...
		if (num_samples > 0) {
			...
			envelope.set_loop_points(instrument_header[228] & 0xFF,
					instrument_header[229] & 0xFF);
			...
			envelope.set_loop_points(instrument_header[231] & 0xFF,
					instrument_header[232] & 0xFF);
	...
	private static Sample read_xm_sample(byte[] sample_headers, int sample_idx,
			DataInput data_input) throws IOException {
		...
		if (sixteen_bit) {
			...
			sample.set_sample_data(decoded_sample_data, loop_start >> 1, loop_length >> 1,
					ping_pong);
		} else {
