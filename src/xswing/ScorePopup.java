...
public class ScorePopup extends SObject{
	...
	public ScorePopup(Font font, int x, int y, String score) {
		...
		transparency = new TransparancySlider(50, timeToLive, TransparancySlider.LINEAR_FADE_OUT);
		size = new ValueSlider(1, 1.3, 20, 50, ValueSliderType.SLIDE_IN);
		//size.setValue(1);
		this.font = font;
		//y-= font.getHeight(score)* size.getValue();
		//FontStringsToSpiteSheetConverter converter = new FontStringsToSpiteSheetConverter(font, new String[]{score});
	...
	public void render(Graphics g) {
		...
		//g.drawString(score, x/(float)size.getValue(), (y/(float)(size.getValue()) - (float)(font.getHeight(score)* size.getValue()))); //- font.getHeight(score)* size.getValue()
		
		g.drawString(score, x, y);
		//font.drawString(x/(float)size.getValue(), (y/(float)(size.getValue()) - (float)(font.getHeight(score)* size.getValue())), score, transparency.getTranparency());
