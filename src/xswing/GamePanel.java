...
public class GamePanel extends BasicGameState implements Resetable{
		...
		if(!multiplayer)
			...
			if(multiPlayer1.getScore() >= scorePlayer1){
				if(coopertive){
					...
					multiPlayer2.fireXSwingEvent(new XSwingEvent(this, GameEventType.PRESSED_DOWN));
				}
			...
			if(multiPlayer2.getScore() >= scorePlayer2){
				if(coopertive){
					...
					multiPlayer1.fireXSwingEvent(new XSwingEvent(this, GameEventType.PRESSED_DOWN));
				}
	...
	private void initGame(boolean multiplayer, GameContainer container, StateBasedGame game) throws SlickException{
